<template>
  <div class="row">
    <div class="col-12 px-0 my-3 text-center">
      ...
      <qrcode-vue
        v-else-if="presentationRequestId"
        :value="presentationRequestURI"
        :margin="1"
        :size="300"
        level="M"
        class="my-3"
        id="presentation-request-canvas"
      />
...
export default {
  ...
  methods: {
      ...
      if (this.presentationRequestId) {
        this.$api
          ...
          .then((res) => {
            ...
            const presentation =
              typeof presentationResponse.vp_token == 'string'
                ? JSON.parse(presentationResponse.vp_token)
                : presentationResponse.vp_token
            if (this.mode == 'authenticate') {
              this.authentication = presentation
            } else {
              this.$store.dispatch('addVerifiables', [presentation]).then(() =>
                this.$router.push({
                  path: '/verify',
                  query: { challenge: this.nonce },
                }),
              )
            }
          })
          .catch((error) => {
            if (error.response.status != 404) {
              this.toast.error(
                `Something went wrong fetching the presentation request!\n${error}`,
              )
              clearInterval(this.intervalId)
              this.intervalId = undefined
            }
    ...
    registerPresentationRequest() {
      ...
      this.$api
        .post(
          this.$store.state.OPENID_ENDPOINT + 'request' + this.getRequestPath(),
        ...
        .catch((error) => {
          this.toast.error(
            `Something went wrong generating the presentation request!\n${error}`,
          )
        })
        .finally(() => {
